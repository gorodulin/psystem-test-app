...
RSpec.describe ChargeTransaction, type: :model do
  ...
  let(:record) { create :charge_transaction }
  ...
  context "on save" do
    it "gets 'error' status if initial transaction is not approved" do
      { "error" => "error",
        ...
        record = create(:charge_transaction,
                         status: "approved",
    ...
    it "fails to create if a duplicate exists" do
      ...
      creation = -> { create(:charge_transaction,
                             authorize_transaction: initial_transaction,
                             merchant: initial_transaction.merchant) }
      expect { creation.call }.to change { described_class.count }.by(1) # with approved status
      expect { creation.call }.to change { described_class.count }.by(1) # with error status
      expect { creation.call }.to raise_error(ActiveRecord::RecordNotUnique) # duplicate
    end
  end
